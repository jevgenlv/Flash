class gfx.controls.ScrollingList extends gfx.controls.CoreList
{
   ...
   function get scrollPositionMax()
   {
      return this._dataProvider.length - this.totalRenderers;
   }
   ...
   function set rowCount(value)
   {
      ...
      this.height = _loc3_ * value + this.margin * 2 + this.paddingTop + this.paddingBottom;
   }
   ...
   function handleInput(details, pathToFocus)
   {
      ...
      {
         _loc6_ = _loc3_.handleInput(details,pathToFocus.slice(1));
         if(_loc6_)
         {
            return true;
         }
   ...
   function draw()
   {
      ...
      else
      {
         ...
         if(_loc3_ == null)
         {
            _loc4_ = this.createItemRenderer(99);
            _loc4_.enableInitCallback = false;
            _loc3_ = _loc4_._height;
            _loc4_.removeMovieClip();
         }
         _loc5_ = this.margin * 2 + this.paddingTop + this.paddingBottom;
         this.totalRenderers = Math.max(0,(this.__height - _loc5_ + 0.05) / _loc3_ >> 0);
         this.drawRenderers(this.totalRenderers);
         this.drawLayout(this.availableWidth,_loc3_);
      }
      this.updateScrollBar();
      this.invalidateData();
      this.setState();
   ...
   function populateData(data)
   {
      ...
      while(_loc2_ < this.renderers.length)
      {
         ...
         _loc3_ = this._scrollPosition + _loc2_;
   ...
   {
      var _loc2_ = Math.max(0,this.dataProvider.length - this.totalRenderers);
      if(this._scrollBar.setScrollProperties != null)
      {
   ...
   function scrollWheel(delta)
   {
      ...
      var _loc2_ = !(this._scrollBar != undefined && this._scrollBar.pageScrollSize != undefined) ? 1 : this._scrollBar.pageScrollSize;
      this.scrollPosition = this._scrollPosition - delta * _loc2_;
   }
   function setState()
   {
